...
function resetCopy(timestamp: number | null, verb = "resets") {
  if (!timestamp || !Number.isFinite(timestamp)) return verb === "renews" ? "no renewal time reported" : "no reset time reported";
...
function Overview({ data, daily, sessions, agent, metricRange, onMetricRangeChange, onSession, accent, providerColors, sceneEffects }: {data:DashboardData;daily:MetricRow[];sessions:Session[];agent:string;metricRange:MetricRange;onMetricRangeChange:(range:MetricRange)=>void;onSession:(session:Session)=>void;accent:string;providerColors:ProviderColors;sceneEffects:SceneEffects}) {
  ...
  const blockElapsed = !activeBlock ? 0 : activeBlock.isActive ? Math.min(100, Math.max(0, (Date.now() - blockStart) / Math.max(1, blockEnd - blockStart) * 100)) : 100;
  const agentTotals = new Map<string, number>();
  daily.forEach((row) => (row.agents ?? [row]).forEach((item) => agentTotals.set(item.agent, (agentTotals.get(item.agent) ?? 0) + item.totalTokens)));
  ...
  return <div className="view-stack page-enter">
    ...
    <section className="dashboard-grid">
      <article className="panel block-panel">
        ...
        {activeBlock ? <>
          <div className="block-ring" role="img" aria-label={`${Math.round(blockElapsed)}% of the five-hour window elapsed; ${formatCompact(activeBlock.totalTokens)} tokens used`} style={{"--progress": `${blockElapsed}%`} as any}><div><strong>{formatCompact(activeBlock.totalTokens)}</strong><span>tokens</span></div></div>
          ...
          <p className="scope-note"><Clock3/> {formatDate(activeBlock.startTime)} → {formatDate(activeBlock.endTime)} · ring shows {activeBlock.isActive ? `${Math.round(blockElapsed)}% of the window elapsed` : "a completed window"}</p>
        </> : <Empty text="No reconstructed blocks found."/>}
      </article>
      <article className="panel agent-panel">
        <div className="panel-heading"><div><span className="overline">AGENT MIX</span><h2>Who used the context?</h2><p>{rangeLabel} · follows the active filters</p></div><Bot/></div>
