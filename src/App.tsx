...
function tooltipModels(
  ...
  return (row?.models?.[provider] ?? []) as Array<{
    name: string;
    tokens: number;
    cost: number;
  }>;
...
function tooltipProjects(row: any): TooltipProject[] {
  ...
  const groups = (row?.projectGroups ?? {}) as Record<
    string,
    ProjectActivity[]
  >;
...
function ProviderChartTooltip({ active, payload, label, coordinate }: any) {
  ...
  return (
    ...
    >
      ...
      {providerSeries
        .map((provider) =>
          payload.find((item: any) => item.dataKey === provider.key),
        )
          ...
          return (
            <section className="tooltip-provider" key={item.dataKey}>
              <div className="tooltip-provider__head">
                ...
                <b>{formatMoney(row?.costs?.[item.dataKey] ?? 0)}</b>
              </div>
